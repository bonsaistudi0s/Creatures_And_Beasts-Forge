...
public class CindershellItemLayer extends RenderLayer<CindershellEntity, AgeableModelProvider<CindershellEntity>> {
    ...
    public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, CindershellEntity entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if (stack != ItemStack.EMPTY) {
            ...
            if (!entitylivingbaseIn.isBaby()) {
                ...
                Minecraft.getInstance().getItemRenderer().renderStatic(stack, TransformType.THIRD_PERSON_LEFT_HAND, packedLightIn, 0, matrixStackIn, bufferIn, 0);
